...
export const defaultApiFn = (apiName: string, path: string, folderInfo, item) => {
  const { parameters, name, method: methodInUpperCase, requestBody } = item
  const method = methodInUpperCase.replace(/^\w/, c => c.toUpperCase()).replace(/(?!^)\w/g, c => c.toLowerCase())
  let apiFun = null
